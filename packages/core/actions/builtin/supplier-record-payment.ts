/**
 * supplier.record_payment Action
 *
 * Record a supplier payment and reduce outstanding balance.
 */
...
export const supplierRecordPaymentAction = defineAction({
  id: 'supplier.record_payment',
  category: 'financial',
  description: 'Record a supplier payment and decrease outstanding balance',
  version: '1.0.0',
  ...
  inputSchema: objectSchema(
    {
      ...
      amount: numberProp('Payment amount', { minimum: 0.01 }),
      description: stringProp('Payment description (optional)'),
    },
    ['supplierId', 'amount'],
    'Input for supplier.record_payment action'
  ),
  ...
  async execute(input, context) {
    try {
      ...
      const newBalance = Math.max(currentBalance - Number(input.amount), 0);
