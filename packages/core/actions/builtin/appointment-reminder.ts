/**
 * appointment.reminder Action
 *
 * Mark an appointment reminder as sent.
 */

import { createClient, SupabaseClient } from '@supabase/supabase-js';
import { defineAction, success, failure, objectSchema, stringProp } from '../helpers';

let cachedSupabase: SupabaseClient | null = null;

function getSupabaseClient(): SupabaseClient {
  if (cachedSupabase) return cachedSupabase;

  const url = process.env.SUPABASE_URL;
  const serviceKey = process.env.SUPABASE_SERVICE_ROLE_KEY;

  if (!url || !serviceKey) {
    throw new Error('Missing SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY');
  }

  cachedSupabase = createClient(url, serviceKey);
  return cachedSupabase;
}

export const appointmentReminderAction = defineAction({
  id: 'appointment.reminder',
  category: 'scheduling',
  description: 'Mark appointment reminder as sent',
  version: '1.0.0',

  inputSchema: objectSchema(
    {
      businessId: stringProp('Business/tenant ID (optional, defaults to context.tenantId)'),
      appointmentId: stringProp('Appointment ID'),
      reminderSentAt: stringProp('Reminder sent time (ISO string, optional)'),
    },
    ['appointmentId'],
    'Input for appointment.reminder action'
  ),

  outputSchema: objectSchema({
    id: { type: 'string', description: 'Appointment ID' },
    reminderSentAt: { type: 'string', description: 'Reminder sent timestamp' },
  }),

  retryable: true,
  idempotent: true,

  async execute(input, context) {
    try {
      const supabase = getSupabaseClient();
      const businessId = (input.businessId as string) || context.tenantId;
      const reminderSentAt = input.reminderSentAt ?? new Date().toISOString();

      const { data: appointment, error } = await supabase
        .from('appointments')
        .update({ reminder_sent_at: reminderSentAt })
        .eq('id', input.appointmentId)
        .eq('business_id', businessId)
        .select('*')
        .single();

      if (error) {
        return failure(`Failed to mark reminder: ${error.message}`, {
          errorCode: 'APPOINTMENT_REMINDER_FAILED',
          shouldRetry: true,
        });
      }

      return success({
        id: appointment.id,
        reminderSentAt: appointment.reminder_sent_at,
      });
    } catch (error) {
      return failure(
        error instanceof Error ? error.message : String(error),
        { errorCode: 'APPOINTMENT_ERROR', shouldRetry: true }
      );
    }
  },
});
